// File: TetrisBoard.cpp
// class: COP 3003 Programming II
// Author: Gabriel Correa
// Description: source file for the class TetrisBoard
//--------------------------------------------------------------------------
#include <iostream>
#include "TetrisBoard.h"
#include "ShapeI.h"
#include "ShapeZ.h"
#include "ShapeJ.h"
#include "ShapeL.h"
#include "ShapeO.h"
#include "ShapeT.h"
#include "MyShape.h"

//local functions declarations
//---------------------------------------------------------------------------
bool isKeyPressed(KeyPressedState input[],sf::Keyboard::Key key);

// Constructors/ Destructors
//---------------------------------------------------------------------------
/**
 * default constructor setups up the game board
 * */
TetrisBoard::TetrisBoard() {

    _counters ={FRAMES_NEW_SHAPE,0,FRAMES_AUTO_MOVE,0,0}; //initialize frame counters

    // set up the game board grid
    //----------------------------------------------------------


    // position and size of a grid block
    sf::Vector2f position{GRID_LEFT,GRID_TOP}; // screen position
    sf::Vector2f size{BLOCK_SIZE,BLOCK_SIZE}; // screen size (pixels)

    //create a loop block shape to update the grid cells
    sf::RectangleShape block{size};
    block.setFillColor(BACKGROUND_COLOR);
    block.setOutlineColor(GRID_COLOR);
    block.setOutlineThickness(-1.f);

    //populate the game board grid with block shapes
    for(int row=GAME_ROWS-1;row>=0;--row){
        for(int col = 0;col<GAME_COLUMNS;++col){
            block.setPosition(position); // set the screen position of block

            _cells[row][col] = { false,block}; //copy shape to grid

            position.x += size.x; //move block right 1 cell

        } // end of columns

        position.x =GRID_LEFT; // rest block left to left side of the grid
        position.y += size.y; //move block down to next row of grid


    } // end of row loop

    //initialize random number generation

    _randGenerator = std::mt19937 (_randDevice());
    _randDistribution = std::uniform_int_distribution<>(Tetromino::ShapeType::SHAPE_NONE+1, Tetromino::ShapeType::SHAPE_COUNT-1);

    // initialize current and next shape properties
    _currentShape = nullptr;
    _nextShape = nullptr;
    _currentCell = sf::Vector2i {0,0};



    nextShape(); // get the next shape



} // default constructor

/**
 * Destructor - removes dynamic memory allocated for the shapes
 */
TetrisBoard::~TetrisBoard() {
    delete _currentShape;
    _currentShape = nullptr;

    delete _nextShape;
    _nextShape = nullptr;
}

// Public Methods
//---------------------------------------------------------------------------
/**
 * Update shape objects on the Tetris board
 * @param input - user keypress
 * @return true if game should end*/
bool TetrisBoard::update(KeyPressedState *input) {
    bool endgame = false;

    if(_currentShape){
        if(isKeyPressed(input,sf::Keyboard::Key::Space)){
            if(canRotateShape()) {
                _currentShape->rotate();
            }
        }

        bool moveDown = false; //true if we are moving the shape down

        if(isKeyPressed(input,sf::Keyboard::Key::S)){
            moveDown = true;
        } //user move down

        //if user requested, or it is time to auto move shape down
        if(moveDown||_counters.autoMove>=_counters.newShapeRate){
            //see if we can move it down first
            if(canMove(Tetromino::Movement::MoveDown)) {
                _currentShape->move(Tetromino::Movement::MoveDown);
                _currentCell.y-=1;
            }
            //reset auto move counter
            _counters.autoMove = 0;
        } // auto or user move down
        else{
            _counters.autoMove++; // otherwise increment frame counter
        }

        //always check to see if the shape could move down again and lock it if it is on top of another shape or the floor
        if(!canMove(Tetromino::Movement::MoveDown)){
            lockShape();
            _counters.numberOfShapes++;
            if(_counters.numberOfShapes % 5 == 0){ // decreases the auto move rate every time 5 shapes were generated
                _counters.autoMoveRate -=5; // the shapes are going to auto move down faster
                if(_counters.autoMoveRate == 0){
                    _counters.autoMoveRate = 1; // it makes sure the auto move rate will not be zero
                }
            }
            delete _currentShape;
            _currentShape = nullptr;
        }

        if(isKeyPressed(input,sf::Keyboard::Key::A)){
            if(canMove(Tetromino::Movement::MoveLeft)) {

                _currentShape->move(Tetromino::Movement::MoveLeft);
                _currentCell.x -= 1;
            }
        }
        else if(isKeyPressed(input,sf::Keyboard::Key::D)){
            if(canMove(Tetromino::Movement::MoveRight)) {
                _currentShape->move(Tetromino::Movement::MoveRight);
                _currentCell.x += 1;
            }
        }

        // key used to print the information of the tetromino in the output console
        if(isKeyPressed(input,sf::Keyboard::Key::Enter)){
            std::cout<<_currentShape->toString()<<std::endl;
        } // current shape exists

    }
    else{ // no current shape
        //count frames until time to show next shape
        if(_counters.newShape<_counters.newShapeRate){
            _counters.newShape++;
        }

        else{ // show next shape

            _counters.newShape = 0; // reset counter
            _currentShape = _nextShape;// copy next to current
            endgame = nextShape(); //get next shape and set the endgame variable to true if there is no space to generate another shape

        }
    } // current shape
    return endgame;
} //end of update

/**
 * draw game objects on the window
 * @param window - main game window
 * */

void TetrisBoard::render(sf::RenderWindow &window) {
    for(int row = 0;row<GAME_ROWS;++row){
        for(int col=0;col<GAME_COLUMNS;++col){

            window.draw(_cells[row][col].block);

        }

    }
    //draw current shape if we have one
    if(_currentShape){
        //set the position of the current shape so it is generated inside the grid
        sf::Vector2f  currentShapePosition = sf::Vector2f((_currentCell.x + 1) *BLOCK_SIZE,(GAME_ROWS - _currentCell.y) * BLOCK_SIZE);
        _currentShape->setPosition(currentShapePosition);
        _currentShape ->draw(window);
    }


    // draw the next shape if it exists
    if (_nextShape) {

        // Draw the next shape
        _nextShape->draw(window);
    }
} //end render

// Private Methods
//---------------------------------------------------------------------------

/**
 * Randomly selects the next shape to show
 * @return true if there is no space for the next shape
 */
bool TetrisBoard::nextShape() {

    //reset current shape cell to top center
    _currentCell = sf::Vector2i(START_CELL_COLUMN,START_CELL_ROW);

    //calculate outside grid position to display next shape
    float x = GRID_LEFT + BLOCK_SIZE * GAME_COLUMNS + 20;
    float y = (GAME_ROWS - _currentCell.y) * BLOCK_SIZE;

    sf::Vector2f position{x,y};


    //generate a shape for nextShape
    auto nextType = static_cast<Tetromino::ShapeType>(_randDistribution(_randGenerator));

    switch(nextType){
        case Tetromino::ShapeType::SHAPE_Z:
            _nextShape = new ShapeZ(position);
            break;
        case Tetromino::ShapeType::SHAPE_J:
            _nextShape = new ShapeJ(position);
            break;
        case Tetromino::ShapeType::SHAPE_I:
            _nextShape = new ShapeI(position);
            break;
        case Tetromino::ShapeType::SHAPE_L:
            _nextShape = new ShapeL(position);
            break;
        case Tetromino::ShapeType::SHAPE_O:
            _nextShape = new ShapeO(position);
            break;
        case Tetromino::ShapeType::SHAPE_S:
            _nextShape = new ShapeO(position);
            break;
        case Tetromino::ShapeType::SHAPE_T:
            _nextShape = new ShapeT(position);
            break;
        case Tetromino::ShapeType::MY_SHAPE:
            _nextShape = new MyShape(position);
            break;



    } //switch type

    //check if there is space to generate the next shape
    if(hasCollision(*_nextShape,_currentCell)){
        return true;
    }
    return false;
}// end next shape

/**
 * Check if the shape is able to move
 * @param movement - direction that the user wants to move the shape
 * @return true if the shape is able to move
 * */
bool TetrisBoard::canMove(Tetromino::Movement movement) {
    bool isAbleToMove = true;

    //make a copy of the current cell
    sf::Vector2i tempCell = _currentCell;

    //move temp cell location
    switch (movement) {
        case Tetromino::MoveDown:
            if(_currentCell.y < 0){
                isAbleToMove = false;
            }
            else{
                tempCell.y -= 1;
            }
            break;
        case Tetromino::MoveRight:   // need +1 for <I> shape
            if(_currentCell.x + _currentShape->getColumns() > GAME_COLUMNS+1){
                isAbleToMove = false;
            }
            else {
                tempCell.x += 1;
            }
            break;
        case Tetromino::MoveLeft:
            if(_currentCell.x<-1){ // need -1 for the shape I once rotated
                isAbleToMove = false;
            }
            else{
                tempCell.x -= 1;
            }

    } // end switch direction
    if(isAbleToMove) {
        // declare temp shape so default constructor is called to initialize empty shape
        Tetromino tempShape;

        // make a copy of the current shape to temp shape
        tempShape = *_currentShape;

        isAbleToMove = !hasCollision(tempShape,tempCell);
    }

    return isAbleToMove;
} // end canMove

/**
 * Lock the current shape into the gameboard
 * */
void TetrisBoard::lockShape() {
    for(int row = 0; row < _currentShape->getRows();++row){
        for(int column = 0; column <_currentShape->getColumns();++column){
            if(_currentCell.x + column >=0 || _currentCell.x+ column <=(GAME_COLUMNS-1)|| _currentCell.y - row >=0){
                if(_currentShape->hasBlock(row,column)){
                    _cells[_currentCell.y - row][_currentCell.x + column].block.setFillColor(_currentShape->getFillColor());
                    _cells[_currentCell.y - row][_currentCell.x + column].is_filled = true;
                }
            }
        } //columns
    } // rows
} // end lock shape

/**
 * See if any of the blocks in a shape overlay a block in the grid or are outside the walls
 * @param shape - reference to the shape to check
 * @param location - location of the shape
 * @return true if there is a collision
 * */
bool TetrisBoard::hasCollision(Tetromino &shape, sf::Vector2i location) {
    bool collision = false;

    // used to indicate if current block in grid is filled or outside the wall
    bool filled{};

    //check if temp shape at new temp cell location has any collisions with filled cells
    for(int row = 0;row < shape.getRows() && !collision;++row){
        for(int column = 0; column <shape.getColumns() && !collision;++column){

            // determine if grid cell is filled
            if(location.x + column < 0 || location.x +column >= GAME_COLUMNS || location.y - row <0){
                filled = true;
            }
            else {
                filled = _cells[location.y-row][location.x+column].is_filled;
            }
            //check for collision between temp shape and grid cell
            if(filled && shape.hasBlock(row,column)){
                collision = true;
            }
        } //columns

    } // rows
    return collision;
}// end has collision

/**
 * Determine if a shape can rotate in its current position and not collide with any existing shapes.
 * The shape may have to perform a wall-kick to put it back inside game board
 * @return true if it can rotate
 * */
bool TetrisBoard::canRotateShape() {
    bool isAbleToRotate= true;

    //make a copy of the current cell
    sf::Vector2i tempCell = _currentCell;

    //make sure location and shape are inside game board grid
    wallKick(tempCell);

    //declare temp shape so default constructor is called to initialize empty shape before copy
    Tetromino tempShape;

    // make a copy of the current shape to temp shape
    tempShape = *_currentShape;

    //rotate the temp shape
    tempShape.rotate();

    isAbleToRotate = !hasCollision(tempShape,tempCell);

    //copy any wall-kicks back to current cell location
    if (isAbleToRotate){
        // see if the moved X between temp and current during a wall-kick
        int diffX = std::abs(tempCell.x - _currentCell.x);
        if(diffX > 0){
            //assume on left wall (x is -) so move right
            auto dir = Tetromino::Movement::MoveRight;

            //if the x was + then on right side
            if(_currentCell.x > 0){
                dir = Tetromino::Movement::MoveLeft;
            }
            _currentShape->move(dir,diffX);
            _currentCell = tempCell;
        }

    }


    return isAbleToRotate;
} // end canRotate()

/**
 * Move the location of a shape back inside the game board
 * @param location - location of the current shape to adjust
 * */
void TetrisBoard::wallKick(sf::Vector2i & location) {
    if(location.x < 0){
        location.x = 0;
    }
    else if(location.x+_currentShape->getColumns()>=GAME_COLUMNS){
        int diff = location.x + _currentShape->getColumns()-GAME_COLUMNS;
        location.x -=diff;
    }
}// end wall kick



//local functions definitions
//---------------------------------------------------------------------------

/**
 * process key input for update frames
 * @param input - current key states
 * @param key - check state of particular key
 * @return true if key is in a pressed state
 * */
bool isKeyPressed(KeyPressedState input[],sf::Keyboard::Key key){
    bool pressed = false;
    // check if key was pressed to rotate the shape
    if(input[key].prior){
        //this is a current  key press for this frame
        if(input[key].current){
            pressed = true;
            //turn off the current flag to indicate we processed it
            input[key].current = false;
        }
        else{ // we were skipping a frame

            //turn off the prior flag, so it can be picked up again
            input[key].prior = false;
        }
    }

    return pressed;

} // is key pressed